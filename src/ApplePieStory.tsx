...
import './ApplePieStory.css';

gsap.registerPlugin(ScrollTrigger);

const ROOT = import.meta.env.BASE_URL + 'apple-scroll/';
const CONFIG = {
  desktop: { count: 96, scrollScreens: 3.5, preloadWorkers: 8 },
  mobile: { count: 84, scrollScreens: 2.4, preloadWorkers: 5 },
} as const;
...
export default function ApplePieStory({ onExplore }: { onExplore: () => void }) {
      ...
      const worker = async () => {
        while (next < config.count && !disposed) {
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            picture.onerror = () => reject(new Error('Apple pie animation missing ' + frameUrl(preferences.variant, index)));
    ...
    load().then(() => {
      ...
      trigger = ScrollTrigger.create({
        ...
        onUpdate: (instance) => {
          ...
          const chapter = currentProgress < .34 ? 0 : currentProgress < .72 ? 1 : 2;
      ...
      if (disposed) return;
      console.warn('CookSmart apple pie animation fallback:', error);
      setStatus('static');
    });
  ...
  return (
    <section ref={outer} id="home" className="kitchen-cinema"
      ...
      aria-label="CookSmart apple pie scroll experience">
      <div ref={scene} className="kitchen-cinema-stage">
        <img className="cinema-poster" src={poster}
          alt="Floating apples, butter, cinnamon and pie crust coming together" />
        ...
        <div className="cinema-top">
          <span>COOKSMART / EXPERIENCE 001</span><span>FROM INGREDIENTS TO INSPIRATION</span>
        </div>
        <div className="cinema-copy">
          <div className="cinema-chapter chapter-one"
            aria-hidden={status === 'ready' && phase !== 0}>
            <p className="cinema-eyebrow">01 / THE IDEA</p>
            <h1>A little<br /><em>magic.</em></h1>
            <p>Apples, cinnamon, and the delicious possibility of what comes next.</p>
            <small>SCROLL TO EXPLORE ↓</small>
          </div>
          <div className="cinema-chapter chapter-two"
            aria-hidden={status !== 'ready' || phase !== 1}>
            <p className="cinema-eyebrow">02 / THE MAKING</p>
            <h2>Made from<br /><em>scratch.</em></h2>
            <p>Watch everyday ingredients come together in an extraordinary way.</p>
          </div>
          <div className="cinema-chapter chapter-three"
            aria-hidden={status !== 'ready' || phase !== 2}>
            <p className="cinema-eyebrow">03 / THE REVEAL</p>
            <h2>Worth the<br /><em>wait.</em></h2>
            <p>From the first ingredient to the final golden crust.</p>
